...
float MinimalDistance = 50;

// short for "show indices for n further frames"
int showIndicesFnFF = 0;


void setup() {
    size(800, 600);
    
    posList.add(new PVector(width/2, height/2));
    showIndicesFnFF = 180;
}
...
void mousePressed() {
    if (mouseButton == LEFT) {
        ...
        Iterator<PVector> it = posList.iterator();
        while (it.hasNext()) {
            distList.append(PVector.dist(it.next(), newPos));
        }
        
        print("Distances to new position: ");
        println(distList);
        
        boolean allAreFarAway = distList.min() > MinimalDistance;
...
void keyPressed() {
    ...
    if (key == 'r' || key == 'R') {
        posList.add(new PVector(random(0,width), random(0,height)));
        showIndicesFnFF = 180;
    }

    // TODO Think of some other keys to add or remove aliens 
    //      to the list here, etc.
    // See https://processing.org/reference/ArrayList.html
}
...
void draw() {
    ...
    if (showIndicesFnFF <= 0) {
        ...
        for (int i = 0; i < posList.size(); i++) {
            drawAlien(posList.get(i));
            textSize(16);
            fill(255);
            text(str(i), posList.get(i).x+20, posList.get(i).y+20);
        }
    }
    ...
    textSize(28);
    fill(255);
    text(str(posList.size()) + " element(s)", 8, height-20);
}


// Draws Ellen, the alien, at the given position pos
void drawAlien(PVector pos) {
    // draw the head
    strokeWeight(2);
    stroke(0);
    fill(#32CD32);
